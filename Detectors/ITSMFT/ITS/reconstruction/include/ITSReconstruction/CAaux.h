...
{
 public:
  Cell(int xx = 0u, int yy = 0u, int zz = 0u, int dd0 = 0, int dd1 = 0, float curv = 0.f,
       std::array<float, 3> n = { 0.f });
...
{
 public:
  Road() : Elements(), N(0) { ResetElements(); }
...
{
 public:
  Doublets(int xx = 0, int yy = 0, float tL = 0.f, float ph = 0.f) : x((int)xx), y((int)yy), tanL(tL), phi(ph) {}
  int x, y;
  float tanL, phi;
};
...
inline float Curvature(float x1, float y1, float x2, float y2, float x3, float y3)
{
  ...
  return 2.f * ((x2 - x1) * (y3 - y2) - (x3 - x2) * (y2 - y1)) *
         invsqrt(((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1)) * ((x2 - x3) * (x2 - x3) + (y2 - y3) * (y2 - y3)) *
